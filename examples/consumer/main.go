...
func main() {
	...
	var err error
	ver := strings.Split(apiVersion, ".")
	// Convert the first segment to an integer (major version)
	apiMajorVersion, err = strconv.Atoi(ver[0])
	if err != nil {
...
func createSubscription(resourceAddress string) (sub pubsub.PubSub, err error) {
	...
	log.Infof("DZK posting subURL=%v", subURL)
	log.Infof("DZK posting endpointURL=%v", endpointURL)
	log.Infof("DZK posting sub=%v", sub)
	var subB []byte
...
func getCurrentState(resource string) {
	...
	rc := restclient.New()
	log.Infof("DZK REST API Get resource=%s, url=%s", resource, url.String())
...
func getEvent(w http.ResponseWriter, req *http.Request) {
	log.Infof("DZK received event request")
...
func ackEvent(w http.ResponseWriter, req *http.Request) {
	log.Infof("DZK received ackEvent request")
	defer req.Body.Close()
	bodyBytes, err := io.ReadAll(req.Body)
	if err != nil {
