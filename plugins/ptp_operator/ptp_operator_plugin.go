			...
			for ptpInterface, s := range eventManager.GetStats(config) { // iface->stats
				...
				case ptpMetrics.MasterClockType:
					...
					case ptp.PtpStateChange:
						...
						if s.IsLastOffsetSet() {
							data = processDataFn(data, eventManager.GetPTPEventsData(s.LastSyncState(), s.LastOffset(), string(ptpInterface), eventType))
						} else {
							log.Debugf("Skipping PTP state change event for %s - lastOffset not populated yet", string(ptpInterface))
						}
				...
				case ptpMetrics.ClockRealTime:
					...
					case ptp.OsClockSyncStateChange:
						if s.IsLastOffsetSet() {
							...
							log.Debugf("Skipping PTP OS clock sync state change event for %s - lastOffset not populated yet", string(ptpInterface))
						}
